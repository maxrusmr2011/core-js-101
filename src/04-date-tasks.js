...
function angleBetweenClockHands(dat) {
  ...
  if (mm === 0) {
    ...
    const hha = hh / 6 + (1 / 6) * (mm / 60);
    const mma = mm / 30;
    res = Math.abs(hha - mma) * Math.PI;
  } else {
    const hha = ((12 - hh) / 6) * Math.PI - ((mm / 60) / 6) * Math.PI;
    const mma = ((60 - mm) / 30) * Math.PI;
    res = Math.round(Math.abs(hha - mma) * (10 ** 16)) / (10 ** 16);
  }
  return res;
}
